...
function convertJsonSchemaToZod(jsonSchema: any): z.ZodType<any> {
  ...
  for (const [key, property] of Object.entries(properties)) {
    ...
    let zodType = z
      ...
      .transform((val) => {
        if (
          prop.description?.includes('input: null') &&
          (val === undefined || val === '')
        ) {
          return null;
        }
  ...
  try {
    ...
    const aiTools = availableTools.map((mcpTool: any) => {
      ...
      return {
        ...
        tool: tool({
          description:
            mcpTool.description || `Execute MCP tool: ${mcpTool.name}`,
          inputSchema: zodSchema,
          execute: async (args: any) => {
            const toolPerf = createPerformanceLogger(
              'mcp-tool',
              `execute_${mcpTool.name}`,
            );
...
export async function getMCPToolByName(
  toolName: string,
  { session, dataStream, requestContext }: MCPToolsProps,
) {
